...
public class AreSentencesSimilar { // https://leetcode.com/problems/sentence-similarity-iii/
	boolean areSentencesSimilar(String sentence1, String sentence2) { // time: O(s1 + s2), space: O(s1 + s2)
		...
		while (bothNotEmpty(queue1, queue2) && queue1.getFirst().equals(queue2.getFirst())) {
			queue1.removeFirst();
			queue2.removeFirst();
		}
		while (bothNotEmpty(queue1, queue2) && queue1.getLast().equals(queue2.getLast())) {
			queue1.removeLast();
			queue2.removeLast();
		}
	...
	private boolean bothNotEmpty(LinkedList<String> queue1, LinkedList<String> queue2) {
